...
int main(int argc, char **argv) {
  ...
  courseNameInput = new Fl_Input(120,
				 ...
				 25,
				 "Course Name: ");
  courseNameInput->maximum_size(16);
  ...
  courseYearInput = new Fl_Input(courseNameInput->x(),
				 ...
				 25,
				 "Test Date: ");
  courseYearInput->maximum_size(16);
  
  // Test Number Input Field
  testNumInput = new Fl_Int_Input(courseNameInput->x(),
				  courseYearInput->y()+30,
				  100,
				  25,
				  "Test Number: ");
  testNumInput->maximum_size(2);
  
  // Test Score Input Field
  testScoreInput = new Fl_Int_Input(courseNameInput->x(),
				    testNumInput->y()+30,
				    100,
				    25,
				    "Total Score: ");
  testScoreInput->maximum_size(3);
...
void create_exam(Fl_Widget *w, void *v) {
  if (fli < 1) {
    fl_alert("Exam creation requires one or more questions.", PPosition);
    return;
  }
  ...
  if (course == NULL || year == NULL || !num || !score) {
    fl_alert("One or more name fields are missing.");
    return;
  }
...
void show_callback() {
  ...
  default:
	...
	if (fnfc.filename(i)[0] != '\0') {
	  fl_filename_relative(relative,
			       sizeof(relative), fnfc.filename(i));
